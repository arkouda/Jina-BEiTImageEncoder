    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param pretrained_model_name_or_path: Can be either:
            - A string, the model id of a pretrained BEit model hosted
                inside a model repo on huggingface.co, e.g., 'microsoft/beit-base-patch16-224-pt22k-ft22k'
        ...
        :param use_default_preprocessing: Whether to use the `base_feature_extractor` on
            ...
            method for details.
        :param output_embedding: Output Embedding Token to use. Supported values:
            "cls" - final hidden state of the CLS token 
            "avg" - mean pool of the final hidden states of the patches
        ...
        self.device = device
        self.preprocessor = BeitFeatureExtractor.from_pretrained(
            self.base_feature_extractor
        )
        self.model = BeitModel.from_pretrained(self.pretrained_model_name_or_path)
    ...
    def encode(self, docs: Optional[DocumentArray], parameters: dict, **kwargs):
        ...
        with torch.inference_mode():
            for batch_docs in document_batches_generator:
                ...
                if self.output_embedding ==  "cls":
                    embeddings = embeddings.poooler_output.cpu().numpy()
                else:
                    embeddings = torch.mean(embeddings.last_hidden_state[:, 1:, :], dim=1).cpu().numpy()
